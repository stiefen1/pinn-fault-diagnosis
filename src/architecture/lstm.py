...
class LSTMFaultEstimator(LearningBasedFaultEstimator):
...
register_model("LSTMFaultEstimator", LSTMFaultEstimator)

if __name__ == "__main__":
	device = torch.accelerator.current_accelerator().type if torch.accelerator.is_available() else "cpu"  # type: ignore
	print(f"Using {device} device")
	model = LSTMFaultEstimator(9, 6, n_samples=3).to(device)
